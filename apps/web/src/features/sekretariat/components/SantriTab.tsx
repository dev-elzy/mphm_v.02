...
export function SantriTab({ onViewDetail, isReadOnly = false, selectedYearId }: SiswaTabProps) {
  ...
  // Ambil columns dan data yang sesuai dengan sub-tab aktif
  const getGridProps = () => {
    switch (activeSubTab) {
      case "aktif":
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="relative overflow-hidden p-6 sm:p-8 bg-linear-to-r from-indigo-500/10 via-blue-500/5 to-transparent border border-indigo-500/20 dark:border-indigo-500/10 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-6 shadow-sm">
        ...
        <div className="flex flex-col gap-1.5 z-10">
          ...
          <p className="text-zinc-555 dark:text-zinc-400 text-sm max-w-xl">
            Administrasi data induk santriwati terintegrasi dengan Nomor Stambuk, NIK, dan alamat wilayah.
          </p>
        </div>
      ...
      {/* Premium Tabs Menu - Varied colors based on active tab */}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      >
                        ...
                        <option value="CUTI">CUTI (Izin Istirahat Sementara)</option>
                        <option value="BOYONG">BOYONG (Keluar Baik)</option>
                        <option value="DROPPED">DROPPED (Keluar Diberhentikan)</option>
                        {newClass === "Aliyyah III-A" && (
                          <option value="KHIDMAH">KHIDMAH (Pengabdian / Al-Robithoh)</option>
                        )}
                        {newClass === "Al-Rabithoh" && (
