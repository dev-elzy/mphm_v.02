...
export function KhidmahTab({ onViewDetail, isReadOnly = false, selectedYearId }: SiswaTabProps) {
  ...
  const [newStatus, setNewStatus] = useState<string>("KHIDMAH");
  ...
  const [newKhidmahPlacement, setNewKhidmahPlacement] = useState("");
  ...
  const handleOpenEdit = (santri: Santri) => {
    ...
    setNewName(santri.name); setNewNik(santri.nik); setNewGender(santri.gender);
    setNewBirthPlace(santri.birthPlace || ""); setNewBirthDate(santri.birthDate || "");
    setNewPhoneNumber(santri.phoneNumber || ""); setAvatarUrl(santri.avatarUrl || null);
    setNewStambuk(santri.stambuk); setNewNis(santri.nis); setNewNisn(santri.nisn || "");
    setNewClass(santri.class); setNewEnrollmentYear(santri.enrollmentYear); setNewGraduationYear(santri.graduationYear || undefined);
    setNewStatus(santri.status as "KHIDMAH"); setNewAddress(santri.address || "");
    setNewGuardianName(santri.guardianName); setNewGuardianNik(santri.guardianNik || "");
    setNewGuardianPhone(santri.guardianPhone); setNewGuardianRelation(santri.guardianRelation);
    setNewFamilyCardNumber(santri.familyCardNumber);
    setNewKhidmahPlacement(santri.khidmahPlacement || "");
  ...
  const handleSaveForm = async (e: React.FormEvent) => {
    ...
    try {
      const payload: Partial<Omit<Santri, "id">> = {
        ...
        khidmahPlacement: newKhidmahPlacement
      };
  ...
  const activeColumns: ColumnDef<Santri, unknown>[] = [
    ...
    },
    {
      accessorKey: "class",
      header: "Kelas Aktif",
      cell: (info) => <span className="font-semibold text-zinc-800 dark:text-zinc-200">{info.getValue() as string}</span>,
    },
    {
      accessorKey: "khidmahPlacement",
      header: "Penempatan Khidmah",
      cell: (info) => <span className="font-semibold text-emerald-700 dark:text-emerald-400 text-sm">{info.getValue() as string || "Belum Ditentukan"}</span>,
    },
    {
      accessorKey: "address",
    ...
    },
    {
      accessorKey: "status",
      header: "Status",
      cell: (info) => <PillBadge label={info.getValue() as string} variant="success" />,
    },
    {
      id: "actions",
      header: "Aksi",
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="relative overflow-hidden p-6 sm:p-8 bg-linear-to-r from-emerald-500/10 via-teal-500/5 to-transparent border border-emerald-500/20 dark:border-emerald-500/10 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-6 shadow-sm">
        ...
        <div className="flex flex-col gap-1.5 z-10">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-white">
            Masa Pengabdian (Khidmah)
          </h1>
          <p className="text-zinc-555 dark:text-zinc-400 text-sm max-w-xl">
            Kelola data penempatan dan status siswi yang sedang dalam masa pengabdian (Khidmah).
          </p>
        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select 
                        ...
                        className="px-3 py-2.5 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 dark:text-zinc-200"
                      >
                        <option value="KHIDMAH">KHIDMAH (Masa Pengabdian)</option>
                        <option value="GRADUATED">GRADUATED (Lulus / Alumni)</option>
                      </select>
                    </div>

                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-bold text-zinc-500 dark:text-zinc-400">Penempatan Khidmah</label>
                      <input 
                        type="text" 
                        value={newKhidmahPlacement} 
                        onChange={(e) => setNewKhidmahPlacement(e.target.value)}
                        placeholder="Contoh: Pengurus Pusat / Cabang A..."
                        className="px-3 py-2.5 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 dark:text-zinc-200"
                      />
                    </div>
              ...
              <div className="p-6 overflow-y-auto space-y-4 text-sm font-medium">
                <table className="w-full border-collapse">
                  <tbody>
                    ...
                    <tr className="border-b border-zinc-100 dark:border-zinc-800/60">
                      <td className="py-2.5 pr-4 font-bold text-zinc-400 dark:text-zinc-500 w-1/3 text-left">Stambuk</td>
                      <td className="py-2.5 text-zinc-800 dark:text-zinc-200 text-left font-mono">{viewingDetail.stambuk || "-"}</td>
                    </tr>
                    <tr className="border-b border-zinc-100 dark:border-zinc-800/60">
                      <td className="py-2.5 pr-4 font-bold text-zinc-400 dark:text-zinc-500 w-1/3 text-left">NIK</td>
                      <td className="py-2.5 text-zinc-800 dark:text-zinc-200 text-left font-mono">{viewingDetail.nik || "-"}</td>
                    </tr>
                    <tr className="border-b border-zinc-100 dark:border-zinc-800/60">
                      <td className="py-2.5 pr-4 font-bold text-zinc-400 dark:text-zinc-500 w-1/3 text-left">Kelas Akhir</td>
                      <td className="py-2.5 text-zinc-800 dark:text-zinc-200 text-left">{viewingDetail.class || "-"}</td>
                    </tr>
                    <tr className="border-b border-zinc-100 dark:border-zinc-800/60">
                      <td className="py-2.5 pr-4 font-bold text-zinc-400 dark:text-zinc-500 w-1/3 text-left">Penempatan Khidmah</td>
                      <td className="py-2.5 text-emerald-700 dark:text-emerald-400 text-left">{viewingDetail.khidmahPlacement || "Belum Ditentukan"}</td>
                    </tr>
                    <tr className="border-b border-zinc-100 dark:border-zinc-800/60">
                      <td className="py-2.5 pr-4 font-bold text-zinc-400 dark:text-zinc-500 w-1/3 text-left">Alamat Wilayah</td>
                      <td className="py-2.5 text-zinc-800 dark:text-zinc-200 text-left">{viewingDetail.address || "-"}</td>
                    </tr>
                    <tr className="border-b border-zinc-100 dark:border-zinc-800/60">
                      <td className="py-2.5 pr-4 font-bold text-zinc-400 dark:text-zinc-500 w-1/3 text-left">Status Keaktifan</td>
